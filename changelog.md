...
* **[Mission Generation]** Hornet and Tomcat squadrons wear sequenced board numbers: each squadron gets a modex block (100, 200, ...; Tomcats first, like a real air wing) and its jets are numbered X00, X01, X02 in order, instead of the random three-digit numbers pydcs deals out. The Tomcat paints its number into the livery rather than drawing `onboard_num`, so a squadron's livery now follows each jet's board number: the livery painted with that number where the set has one, the CAG / hi-vis livery for the X00 jet only, and the line liveries in turn for the rest. (#874)
* **[UX]** The Payload tab can pin a Hornet or Tomcat flight's board number: the lead takes the number entered and the wingmen follow in order (105, 106, 107, 108). Taking a number another flight has pinned moves that flight to the next free numbers, and the tab says where; no other aircraft is given a pinned number. (#874)
* **[Data]** F-14B(U) squadrons carry every DCS livery for the squadron as a livery set instead of two single-livery High Vis / Low Vis presets, so jets in one squadron no longer all show the same painted board number. (#874)
