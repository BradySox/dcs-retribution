...
from game.missiongenerator.dtc.elevations import (
    control_point_elevation,
...
#: Waypoint types sitting on a field: takeoff and landing take that field's own
#: elevation. DIVERT is excluded because an off-map one is an exit vector planned
#: at cruise, and the two are not separable here.
...
def steerpoint_elevation(waypoint: FlightWaypoint, game: Game) -> float:
    ...
    Takeoff and landing take their field's elevation (a boat or a FOB has none
    and reads 0). Everything else takes the nearest airfield's, which is an
    estimate -- see :mod:`.elevations`.
    """
    if waypoint.waypoint_type in _FIELD_ELEVATION_WAYPOINTS:
        if waypoint.control_point is None:
            return 0.0
        return control_point_elevation(game, waypoint.control_point) or 0.0
    return nearest_field_elevation(game, waypoint.position)


def leg_altitude(waypoint: FlightWaypoint, game: Game) -> tuple[float, int]:
    """The altitude to fly the leg, metres MSL, plus the DTC altitudeType (always 1).

    Goes in the Viper's ``routeAltitude`` and the Hornet's ``NAV_ROUTE`` entry,
    never in the point's own ``alt``. The Viper's DED shows this number as the
    steerpoint ELEV, and nothing honours the AGL tag: the editor's
    ``transformAltitude`` in ``MPD/NAV_Routes.lua`` returns its input, so a point
    written as "0 AGL" reads ELEV 0 in the jet. Every altitude is therefore
    written MSL.
    ...
    sets 0 AGL so pods and weapons can be slaved to them), so a flyover's
    altitude is the ground estimate. An AGL-planned leg (a low-level or
    helicopter profile) is converted with the same estimate.
    """
    if waypoint.flyover:
