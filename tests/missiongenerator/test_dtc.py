...
midnight, SA/HSD elements, and the steerpoint elevations.
"""
...
from game.missiongenerator.dtc import DtcGenerator, elevations
...
from game.missiongenerator.dtc.elevations import field_elevation
...
def _airbase_cp(
    ...
    *,
    ...
    # A boat or a FOB has no pydcs airport, and so no elevation on record.
    if airport_id is not None:
        cp.airport = SimpleNamespace(id=airport_id)
    return cp


def _elevations(monkeypatch: pytest.MonkeyPatch, table: dict[str, float]) -> None:
    """Stand in for the shipped airfield table, keyed by the fake airport ids."""
    monkeypatch.setattr(
        elevations,
        "field_elevation",
        lambda terrain, airport_id: table.get(str(airport_id)),
    )
...
def test_viper_destinations_lead_with_the_divert(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    ones that fit."""
    _elevations(monkeypatch, {"kobuleti": 17.0})
    flight, mission_data, game = _viper_with_fields(
        [
            _airbase_cp("Vaziani", 200000, 200000),
            _airbase_cp("Kobuleti", 61000, 81000, airport_id="kobuleti"),
    ...
    # A field with an elevation on record carries it; one without reads 0.
...
def test_a_steerpoints_alt_is_its_ground_not_its_leg_altitude(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    in the Viper's ``NAV_PTS.lua`` and the Hornet's ``WYPT_NAV.lua`` -- and the
    second is what the jet flies, which the Viper's DED shows as the steerpoint
    ELEV. Writing one number into both told the jet the ground under an 18,000 ft
    nav point was at 18,000 ft, and that a target sat at sea level.
    """
    _elevations(monkeypatch, {"kutaisi": 45.0})
    field = _airbase_cp("Kutaisi", 0, 0, airport_id="kutaisi")
    takeoff = _waypoint(
        "TAKEOFF", FlightWaypointType.TAKEOFF, 0, 0, 0, None, alt_type="RADIO"
    )
    ...
    game = _game(controlpoints=[field])
    ...
    # Nav point: the nearest field's ground, never the 6705 m it is flown at.
    assert nav_pts[0]["alt"] == pytest.approx(45.0)
    assert route["STPT1"]["alt"] == 6705 and route["STPT1"]["altitudeType"] == 1
    # Target: a flyover the .miz puts on the deck, so the leg IS the ground,
    # written MSL because the jet does not resolve an AGL tag.
    assert nav_pts[1]["alt"] == pytest.approx(45.0)
    assert route["STPT2"]["alt"] == pytest.approx(45.0)
    assert route["STPT2"]["altitudeType"] == 1
    # Landing: the field's own elevation, and 0 AGL over it is the field.
    assert nav_pts[2]["alt"] == pytest.approx(45.0)
    assert route["STPT3"]["alt"] == pytest.approx(45.0)
    ...
    assert steerpoints[0]["alt"] == pytest.approx(45.0)
    assert steerpoints[0]["routeAltitude"] == 6705
    assert steerpoints[1]["alt"] == pytest.approx(45.0)
    assert steerpoints[1]["routeAltitude"] == pytest.approx(45.0)
    assert steerpoints[1]["altitudeType"] == 1
    assert steerpoints[2]["alt"] == pytest.approx(45.0)


def test_en_route_elevation_is_the_nearest_fields(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The only height data the campaign carries is per airfield, so a
    steerpoint reads the nearest one's: an estimate, but closer than 0."""
    _elevations(monkeypatch, {"kutaisi": 45.0, "senaki": 12.0})
    kutaisi = _airbase_cp("Kutaisi", 0, 0, airport_id="kutaisi")
    game = _game(
        controlpoints=[
            kutaisi,
            _airbase_cp("Senaki", 60000, 80000, airport_id="senaki"),
            # Nearest to the hold, but with no record: it must not answer 0.
            _airbase_cp("Sukhumi", 1500, 1500, airport_id="sukhumi"),
            _sam_cp(),
        ]
    )
    hold = _waypoint("HOLD", FlightWaypointType.LOITER, 1000, 1000, 6000, None)
    target = _waypoint("TGT", FlightWaypointType.TARGET_POINT, 62000, 84000, 0, None)
    assert steerpoint_elevation(hold, game) == 45.0
    assert steerpoint_elevation(target, game) == 12.0
    # A field takes its own record; a boat has none and is sea level.
    landing = _waypoint(
        "LAND", FlightWaypointType.LANDING_POINT, 0, 0, 0, None, control_point=kutaisi
    )
    assert steerpoint_elevation(landing, game) == 45.0
    trap = _waypoint(
        "LAND",
        FlightWaypointType.LANDING_POINT,
        0,
        ...
        0,
        None,
        control_point=_airbase_cp("CVN-71", 0, 0),
    )
    assert steerpoint_elevation(trap, game) == 0.0
...
def test_a_ground_marked_target_carries_the_ground_as_its_altitude(
    ...
    """The Viper's DED shows routeAltitude as the steerpoint ELEV, and the
    editor's transformAltitude never adds terrain to an AGL value. So a flyover
    target carries the ground estimate in MSL, and an AGL-planned leg is
    converted the same way."""
    _elevations(monkeypatch, {"kirkuk": 700.0})
    flight, mission_data, game = _hornet_fixture()
    game.theater.controlpoints = [_airbase_cp("Kirkuk", 0, 0, airport_id="kirkuk")]
    flight.aircraft_type = SimpleNamespace(dcs_unit_type=SimpleNamespace(id="F-16C_50"))
    flight.waypoints = [
        _waypoint("TAKEOFF", FlightWaypointType.TAKEOFF, 0, 0, 0, None),
        _waypoint(
...
def test_field_elevation_reads_the_shipped_table() -> None:
    elevations._tables.cache_clear()
    with elevations.ELEVATIONS_RESOURCE_PATH.open(encoding="utf-8") as fh:
        shipped = json.load(fh)["terrains"]
    airport_id, metres = next(iter(shipped["Caucasus"].items()))
    assert field_elevation("Caucasus", int(airport_id)) == pytest.approx(metres)
    assert field_elevation("Caucasus", "no such id") is None
    assert field_elevation("NoSuchTerrain", airport_id) is None


def test_the_shipped_elevation_table_matches_pydcs() -> None:
    """Every terrain and airport id in the table resolves in pydcs, so a renamed
    terrain or a renumbered airport fails here instead of silently dropping a
    field's elevation."""
    import dcs.terrain
    from dcs.terrain.terrain import Terrain

    # Keyed by Terrain.name, which is what the generator looks up at runtime;
    # the class names differ from it for some terrains (GermanyColdWar,
    # Sinai).
    by_name = {}
    for attr in dir(dcs.terrain):
        candidate: Any = getattr(dcs.terrain, attr)
        if (
            isinstance(candidate, type)
            and issubclass(candidate, Terrain)
            and candidate is not Terrain
        ):
            # Concrete terrains take no constructor arguments.
            terrain = candidate()  # type: ignore[call-arg]
            by_name[terrain.name] = terrain

    elevations._tables.cache_clear()
    tables = elevations._tables()
    assert len(tables) >= 13
    for terrain_name, table in tables.items():
        assert terrain_name in by_name, f"{terrain_name}: no such pydcs terrain"
        known = {str(airport.id) for airport in by_name[terrain_name].airports.values()}
        unknown = set(table) - known
        assert not unknown, f"{terrain_name}: ids not in pydcs: {sorted(unknown)}"
        assert table, terrain_name
